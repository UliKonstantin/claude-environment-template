...
ExecStart=/var/www/myapp/backend/.venv/bin/uvicorn app.main:app --host 0.0.0.0 --port 8000 --workers 4
Restart=always
RestartSec=5
...
2. **Clone repository**
   ```bash
   git clone https://github.com/user/habit-tracker /var/www/myapp
   ```
...
# Create spec file
pyi-makespec --onefile --name myapp backend/app/main.py
```
